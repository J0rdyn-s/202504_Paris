...
function updateDailyNote() {
  const container = document.getElementById("dailyNoteContainer");
  if (!container || !currentDate || !dailyNotes.length) return;

  const entry = dailyNotes.find((e) => e.date === currentDate);
  const noteKey = currentLanguage === "kr" ? "note_kr" : "note_en";
  const noteLines = entry && entry[noteKey] ? entry[noteKey] : [];

  container.innerHTML = noteLines.length ? `<div class="note-box">${noteLines.map((line) => `<p>${line}</p>`).join("")}</div>` : "";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("json/page_meta.json")
    ...
    .then(async (data) => {
      ...
      // ✅ Load daily notes once
      fetch("json/daily_notes.json")
        .then((res) => res.json())
        .then((data) => {
          dailyNotes = data;
          updateDailyNote();
  ...
  languageSelect.addEventListener("change", async (e) => {
    ...
    if (currentDate) {
      updateDateDisplay(currentDate);
      loadSchedule(currentDate);
      updateDailyNote(); // ✅ language change
    }
  ...
  function buildDateList() {
    ...
    dates.forEach((date, index) => {
      ...
      if (date === todayStr) li.classList.add("today");
      li.onclick = () => {
        mobileList.style.display = "none";
        loadSchedule(date);
        updateDateDisplay(date);
        updateDailyNote(); // ✅ on mobile list click
      };
